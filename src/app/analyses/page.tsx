...
export default function AnalysesPage() {
  ...
  if (opened && openedDraft) return <Home draft={openedDraft} precomputed={opened.result} onBack={{ label: "Saved analyses", onClick: () => setOpened(null) }} />;

  return <div className="app-shell"><Sidebar active={active} setActive={setActive} /><main className="main-content"><header className="topbar"><div className="breadcrumb"><strong>SAVED ANALYSES</strong></div></header><div className="content-wrap">
    ...
    {!loading && !error && items.length === 0 && <section className="signal-panel state-page-panel"><div className="signal-icon"><Archive size={17} /></div><span className="eyebrow">NOTHING SAVED YET</span><h3>No analyses have been saved.</h3><p>Analyses appear here when you upload a project PDF on the document analyzer, confirm its values, and choose <strong>Save analysis</strong> on the report.</p><button className="dark-button" onClick={() => router.push("/documents")}>Open document analyzer <ArrowUpRight size={14} /></button></section>}
    ...
    {!loading && !error && items.length > 0 && <section className="portfolio-list table-panel analyses-list" aria-live="polite"><div className="portfolio-list-header"><strong>{items.length} saved {items.length === 1 ? "analysis" : "analyses"}</strong><span>Newest first</span></div>
      ...
      {items.map((item) => <div className="analyses-row" key={item.id}>
        <button className="analyses-open" onClick={() => void open(item)} disabled={openingId === item.id} aria-label={`Open ${item.name}`}>
          ...
          <span className="analyses-risk"><strong>{item.risk_percentage}<small>/100</small></strong><i className={`risk-pill ${RISK_CLASS[item.risk_level] ?? "stable"}`}><i />{item.risk_level}</i></span>
