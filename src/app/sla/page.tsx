...
import { formatMeasure, severityClass } from "@/components/intelligence/sla-rules-panel";
import { getSlaBreaches, type SlaBreachListResponse, type SlaRuleId } from "@/lib/prediction-api";
...
export default function SlaPage() {
  ...
  const [ruleFilter, setRuleFilter] = useState<SlaRuleId | null>(null);
  ...
  return <div className="app-shell"><Sidebar active={active} setActive={setActive} /><main className="main-content"><header className="topbar"><div className="breadcrumb"><strong>SLA MONITORING</strong></div></header><div className="content-wrap">
    ...
    {summary && <>
      ...
      {listReady && <section className="portfolio-list table-panel sla-list" aria-live="polite">
        ...
        {data.rows.map((row) => { const shown = row.filter_rule ?? row.worst_rule; return <button className="sla-row" key={row.project_id} onClick={() => router.push(`/projects/${encodeURIComponent(row.project_id)}`)} aria-label={`Open ${row.project_id} intelligence report`}>
          ...
          <span className={`risk-pill ${severityClass(shown.severity)}`}><i />{shown.severity}</span>
